    ...
    if (call_does_dispatch && site_count > 0 && UseTypeProfile) {
      ...
      if (receiver_method != nullptr) {
        ...
        if (hit_cg != nullptr) {
          ...
          bool polymorphic_recompile = PolymorphicInlining && next_hit_cg != nullptr;
          ...
          for (int i = 2; i < polymorphic_devirtualize; i++) {
            ciMethod*receiver_method_devirtual = callee->resolve_invoke(jvms->method()->holder(),
                                                                profile.receiver(i));
            if (receiver_method_devirtual != nullptr && !(receiver_method_devirtual->is_native() && cg_intrinsic)) {
              hit_cg_devirtual[i] = this->call_generator(
                  receiver_method_devirtual,
                                             vtable_index, !call_does_dispatch, jvms,
                                             false, prof_factor);
              if (hit_cg_devirtual[i] == nullptr) {
          ...
          if ((morphism == 1 || (morphism == 2 && next_hit_cg != nullptr) || polymorphic_recompile) &&
            ...
            if (PolymorphicInlining && cg_intrinsic != nullptr) {
